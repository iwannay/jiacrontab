...
GOTEST=$(GOCMD) test
GOGET=$(GOCMD) get
BINARY_ADMIN=jiacrontab_admin
BINARY_NODE=jiacrontabd
...
build:
	...
	mkdir $(ADMINDIR)
	mkdir $(NODEDIR)
	cp jiacrontab_admin/jiacrontab_admin.ini $(ADMINDIR)
	cp -r jiacrontab_admin/template $(ADMINDIR)
	cp -r jiacrontab_admin/static $(ADMINDIR)
	cp jiacrontabd/jiacrontabd.ini $(NODEDIR)
